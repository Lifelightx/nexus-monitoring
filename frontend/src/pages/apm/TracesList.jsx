...
const TracesList = () => {
    ...
    const fetchTraces = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/api/otel/traces?${params.toString()}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const getStatusColor = (trace) => {
        // Heuristic: check if any span has error or status != OK
        // For now, assuming standard OTel status mapping if available, or duration threshold
                        ...
                        {loading ? (
                            ...
                            traces.map((trace, index) => (
                                ...
                                >
                                    ...
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-semibold ${trace.TotalDurationMs > 500 ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
